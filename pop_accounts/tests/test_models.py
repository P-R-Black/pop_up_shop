...
from pop_accounts.models import (
    PopUpCustomer,PopUpBid, CustomPopUpAccountManager, SoftDeleteManager, PopUpCustomerAddress, 
    PopUpPurchase, ActiveUserManager, AllUserManager, SoftDeleteUserManager)
...
class TestActiveUserManager(TestCase):
    ...
    def setUp(self):
        """Create test users"""
        # Active user
        self.active_user = PopUpCustomer.all_objects.create(
        ...
        # Deleted user
        self.deleted_user = PopUpCustomer.all_objects.create(
    ...
    def test_get_queryset_excludes_deleted_users(self):
        ...
        users = PopUpCustomer.objects.all()
    ...
    def test_filter_on_active_users_only(self):
        ...
        users = PopUpCustomer.objects.filter(first_name="Active")
        
        self.assertEqual(users.count(), 1)
        self.assertEqual(users.first(), self.active_user)
    ...
    def test_get_on_deleted_user_raises_does_not_exist(self):
        ...
        with self.assertRaises(ObjectDoesNotExist):
            PopUpCustomer.objects.get(email="deleted@example.com")
    
    def test_count_excludes_deleted_users(self):
        """Test that count() excludes deleted users"""
        self.assertEqual(PopUpCustomer.objects.count(), 1)
...
class TestAllUserManager(TestCase):
    ...
    def test_get_queryset_includes_all_users(self):
        ...
        users = PopUpCustomer.all_objects.all()
    ...
    def test_filter_can_find_deleted_users(self):
        ...
        users = PopUpCustomer.all_objects.filter(email="deleted@example.com")
...
class TestCustomPopUpAccountManager(TestCase):
    ...
    def test_create_user_with_all_fields(self):
        ...
        user = PopUpCustomer.objects.create_user(
    ...
    def test_create_user_with_extra_fields(self):
        ...
        self.assertEqual(user.shoe_size, "10")
        self.assertEqual(user.favorite_brand, "nike")
    ...
    def test_create_superuser_with_all_fields(self):
        ...
        superuser = PopUpCustomer.objects.create_superuser(
    ...
    def test_create_superuser_with_is_staff_false_raises_error(self):
        ...
        with self.assertRaises(ValueError) as context:
            PopUpCustomer.objects.create_superuser(
                email="admin@example.com",
                first_name="Admin",
                last_name="User",
                password="pass123",
    ...
    def test_get_by_natural_key_finds_user(self):
        ...
        found_user = PopUpCustomer.objects.get_by_natural_key("natural@example.com")
    ...
    def test_get_by_natural_key_case_sensitive(self):
        ...
        from django.core.exceptions import ObjectDoesNotExist
        with self.assertRaises(ObjectDoesNotExist):
            PopUpCustomer.objects.get_by_natural_key("TEST@example.com")
    ...
    def test_get_by_natural_key_nonexistent_raises_error(self):
        ...
        with self.assertRaises(ObjectDoesNotExist):
            PopUpCustomer.objects.get_by_natural_key("nonexistent@example.com")
    ...
    def test_get_by_natural_key_excludes_deleted_users(self):
        ...
        from django.core.exceptions import ObjectDoesNotExist
        with self.assertRaises(ObjectDoesNotExist):
            PopUpCustomer.objects.get_by_natural_key("deleted@example.com")
...
class TestSoftDeleteUserManager(TestCase):
    ...
    def setUp(self):
        ...
        # Active address
        self.active_address = PopUpCustomerAddress.objects.create(
        ...
        # Deleted address
        self.deleted_address = PopUpCustomerAddress.all_objects.create(
            customer=self.customer,
...
class TestManagerIntegration(TestCase):
    ...
    def test_soft_delete_removes_from_default_manager(self):
        ...
        self.assertEqual(PopUpCustomer.objects.filter(email="test@example.com").count(), 1)
        
        # Soft delete
        user.soft_delete()
        
        # Should not be in default manager
        self.assertEqual(PopUpCustomer.objects.filter(email="test@example.com").count(), 0)
        
        # But should be in all_objects
        self.assertEqual(PopUpCustomer.all_objects.filter(email="test@example.com").count(), 1)
...
class TestPopUpCustomerModel(TestCase):
    ...
    def test_email_is_unique(self):
        ...
        with self.assertRaises(IntegrityError):
            PopUpCustomer.objects.create(
                email="testMail@gmail.com",  # Duplicate
                first_name="Another",
                last_name="User"
            )
    ...
    def test_optional_fields_can_be_blank(self):
        ...
        user = PopUpCustomer.objects.create(
            email="minimal@example.com",
        ...
        self.assertIsNone(user.shoe_size)
    
    def test_default_values(self):
        """Test that fields have correct default values"""
        user = PopUpCustomer.objects.create(email="defaults@example.com")
        
        self.assertTrue(user.mobile_notification)
        self.assertEqual(user.size_gender, 'male')
        self.assertEqual(user.favorite_brand, 'nike')
    ...
    def test_username_field_is_email(self):
        ...
        self.assertEqual(PopUpCustomer.USERNAME_FIELD, 'email')
    ...
    def test_soft_delete_user_not_returned_by_default_manager(self):
        """Test that soft-deleted users are not in default queryset"""
        self.customer.soft_delete()
        users = PopUpCustomer.objects.filter(email="testMail@gmail.com")
        self.assertEqual(users.count(), 0)
    
    def test_soft_delete_user_returned_by_all_objects_manager(self):
        """Test that soft-deleted users are in all_objects queryset"""
        self.customer.soft_delete()
        users = PopUpCustomer.all_objects.filter(email="testMail@gmail.com")
        self.assertEqual(users.count(), 1)
        self.assertTrue(users.first().is_deleted)
    ...
    def test_restore_non_deleted_user_is_safe(self):
        """Test that restoring a non-deleted user doesn't break anything"""
    ...
    def test_prods_interested_in_relationship(self):
        """Test ManyToMany relationship with interested products"""
        # Create a product (adjust based on your PopUpProduct model)
        product = create_test_product_one()
        
        
        self.customer.prods_interested_in.add(product)
        
        self.assertEqual(self.customer.prods_interested_in.count(), 1)
        self.assertIn(product, self.customer.prods_interested_in.all())
    
    def test_prods_on_notice_for_relationship(self):
        """Test ManyToMany relationship with notified products"""
        product = create_test_product_one()
        
        self.customer.prods_on_notice_for.add(product)
        
        self.assertEqual(self.customer.prods_on_notice_for.count(), 1)
        self.assertIn(product, self.customer.prods_on_notice_for.all())
    
    def test_multiple_products_interested_in(self):
        """Test user can be interested in multiple products"""
        product1 = create_test_product_one()
        product2 = create_test_product_two()
        
        self.customer.prods_interested_in.add(product1, product2)
        
        self.assertEqual(self.customer.prods_interested_in.count(), 2)
    ...
    def test_str_with_no_names_returns_none_none(self):
        """Test __str__ method when names are not set"""
        user = PopUpCustomer.objects.create(email="noname@example.com")
        self.assertEqual(str(user), "None None")
    
    def test_str_with_only_first_name(self):
        """Test __str__ method with only first name"""
        user = PopUpCustomer.objects.create(
            email="firstname@example.com",
            first_name="John"
        )
        self.assertEqual(str(user), "John None")
    ...
    def test_size_gender_choices(self):
        """Test that size_gender accepts valid choices"""
        self.customer.size_gender = 'female'
        self.customer.save()
        self.customer.refresh_from_db()
        self.assertEqual(self.customer.size_gender, 'female')
    
    def test_favorite_brand_choices(self):
        """Test that favorite_brand accepts valid choices"""
        valid_brands = ['adidas', 'nike', 'jordan', 'yeezy', 'gucci']
        
        for brand in valid_brands:
            self.customer.favorite_brand = brand
            self.customer.save()
            self.customer.refresh_from_db()
            self.assertEqual(self.customer.favorite_brand, brand)
    
    # ==================== Timestamp Tests ====================
    
    def test_created_timestamp_auto_set(self):
        """Test that created timestamp is automatically set"""
        self.assertIsNotNone(self.customer.created)
    
    def test_updated_timestamp_auto_updates(self):
        """Test that updated timestamp updates on save"""
        original_updated = self.customer.updated
        
        # Make a change and save
        self.customer.first_name = "Updated"
        self.customer.save()
        
        self.customer.refresh_from_db()
        self.assertGreater(self.customer.updated, original_updated)
    
    def test_created_does_not_change_on_update(self):
        """Test that created timestamp doesn't change on updates"""
        original_created = self.customer.created
        
        self.customer.first_name = "Updated"
        self.customer.save()
        
        self.customer.refresh_from_db()
        self.assertEqual(self.customer.created, original_created)
    
    # ==================== Manager Tests ====================
    
    def test_objects_manager_excludes_deleted_users(self):
        """Test default manager excludes soft-deleted users"""
        active_user = PopUpCustomer.objects.create(email="active@example.com")
        deleted_user = PopUpCustomer.objects.create(email="deleted@example.com")
        deleted_user.soft_delete()
        
        all_users = PopUpCustomer.objects.all()
    ...
    def test_all_objects_manager_includes_deleted_users(self):
        ...
        all_users = PopUpCustomer.all_objects.all()
        self.assertIn(active_user, all_users)
    ...
    def test_verbose_name(self):
        ...
        self.assertEqual(
            PopUpCustomer._meta.verbose_name,
            'PopUpCustomer'
        )
    ...
    def test_verbose_name_plural(self):
        ...
        self.assertEqual(
            PopUpCustomer._meta.verbose_name_plural,
            'PopUpCustomers'
        )
    ...
    def test_stripe_customer_id_can_be_stored(self):
        """Test that Stripe customer ID can be stored"""
        self.customer.stripe_customer_id = "cus_1234567890"
        self.customer.save()
        self.customer.refresh_from_db()
        self.assertEqual(self.customer.stripe_customer_id, "cus_1234567890")
    ...
    def test_last_password_reset_can_be_set(self):
        ...
        self.customer.last_password_reset = reset_time
        self.customer.save()
        self.customer.refresh_from_db()
        
        self.assertEqual(self.customer.last_password_reset, reset_time)



class TestPopUpCustomerAddressModel(TestCase):
    """Comprehensive tests for PopUpCustomerAddress model"""
    ...
    def setUp(self):
        ...
        self.address = create_test_address(
            customer=self.customer, first_name="John", last_name="Doe", 
    ...
    def test_address_creation_with_all_fields(self):
        ...
        self.assertEqual(address.first_name, "Robert")
        self.assertEqual(address.middle_name, "James")
        self.assertEqual(address.last_name, "Smith")
    ...
    def test_str_representation(self):
        ...
        expected = "John Doe - 123 Main St, Springfield"
    ...
    def test_str_with_customer_no_names(self):
        ...
        customer_no_name = PopUpCustomer.objects.create(
    ...
    def test_customer_address_related_name(self):
        ...
        addresses = self.customer.address.all()
        self.assertEqual(addresses.count(), 2)
    ...
    def test_customer_address_filtering(self):
        ...
        il_addresses = self.customer.address.filter(state="IL")
...
class TestPopUpBidModel(TestCase):
    ...
    def setUpTestData(cls):
        cls.product, cls.color_spec, cls.size_spec, cls.user1, cls.user2 =  create_seed_data()
    ...
    def test_bid_creation_with_all_fields(self):
        ...
        self.assertEqual(bid.customer, self.user1)
    ...
    def test_customer_relationship(self):
        ...
        self.assertEqual(bid.customer, self.user1)
        self.assertIn(bid, self.user1.bids.all())
...
class TestPopUpPurchaseModel(TestCase):
    ...
    def test_purchase_creation_with_all_fields(self):
        ...
        self.assertEqual(purchase.customer, self.user1)
    ...
    def test_customer_relationship(self):
        ...
        self.assertEqual(purchase.customer, self.user1)
        self.assertIn(purchase, self.user1.purchases.all())
    ...
    def test_multiple_purchases_per_customer(self):
        ...
        customer_purchases = self.user1.purchases.all()
    ...
    def test_can_delete_customer_after_deleting_purchases(self):
        ...
        from django.core.exceptions import ObjectDoesNotExist
        with self.assertRaises(ObjectDoesNotExist):
            PopUpCustomer.all_objects.get(id=self.user1.id)
    ...
    def test_purchase_with_different_customer_than_bid(self):
        ...
        purchase = PopUpPurchase.objects.create(
            customer=self.user2,  # Different customer
            product=self.product,
            bid=self.bid,  # self.bid.customer = self.user1
            price=Decimal('250.00')
        )
        
        self.assertEqual(purchase.customer, self.user2)
        self.assertEqual(self.bid.customer, self.user1)
    ...
    def test_get_customer_purchase_history(self):
        ...
        history = self.user1.purchases.all()
    ...
    def test_get_unpaid_purchases_for_customer(self):
        ...
        unpaid = self.user1.purchases.filter(paid=False)
